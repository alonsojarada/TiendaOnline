<x-app-layout>
    ...
    <div class="py-4">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-4">
            ...
            @if (session('success'))
                <div
                    class="font-medium text-sm text-green-600 dark:text-green-400 bg-green-100 dark:bg-green-900/30 p-3 rounded-xl">
                    {{ session('success') }}
                </div>
            @endif
            ...
            <!-- Tarjeta Principal -->
            <div
                class="bg-white dark:bg-gray-800 shadow-sm rounded-2xl p-5 sm:p-6 border border-gray-100 dark:border-gray-700 text-gray-800 dark:text-gray-200">

                <!-- Encabezado con Cliente, Concepto, Botón PDF y Botón Eliminar -->
                <div
                    class="flex flex-col sm:flex-row justify-between items-start sm:items-center pb-4 border-b dark:border-gray-700 gap-3 mb-4">
                    <div>
                ...
                <div
                    class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-0.5 mb-4 bg-gray-50/50 dark:bg-gray-900/20 px-3.5 py-2 rounded-xl border border-gray-100 dark:border-gray-700/50">
                    <div class="space-y-0.5">
                        <div
                            class="flex justify-between items-center py-0.5 border-b border-gray-200/40 dark:border-gray-700/40">
                        ...
                        <div
                            ...
                            <span
                                class="font-bold text-gray-900 dark:text-white text-base leading-tight">{{ number_format($loan->interest_rate ?? 0, 1) }}
                                %</span>
                        </div>
                        <div
                            class="flex justify-between items-center py-0.5 border-b border-gray-200/40 dark:border-gray-700/40">
                            ...
                            <span
                                class="font-bold text-gray-900 dark:text-white text-base leading-tight">${{ number_format($valorInteresTotal, 2) }}</span>
                        </div>
                        <div class="flex justify-between items-center py-0.5">
                            <span class="text-sm text-gray-500 dark:text-gray-400 font-medium">Total prestado
                                (Capital)</span>
                            <span
                                class="font-bold text-gray-900 dark:text-white text-base leading-tight">${{ number_format($capitalPrestado, 2) }}</span>
                        </div>
                    </div>
                    ...
                    <div class="space-y-0.5">
                        <div
                            ...
                            <span
                                class="font-bold text-gray-900 dark:text-white text-base leading-tight">{{ $cuotasPagadas }}
                                / {{ $totalCuotas }}</span>
                        </div>
                        <div
                            class="flex justify-between items-center py-0.5 border-b border-gray-200/40 dark:border-gray-700/40">
                            <span class="text-sm text-gray-500 dark:text-gray-400 font-medium">Prestado +
                                intereses</span>
                            <span
                                class="font-bold text-gray-900 dark:text-white text-base leading-tight">${{ number_format($montoTotalConInteres, 2) }}</span>
                        </div>
                        <div
                            class="flex justify-between items-center py-0.5 border-b border-gray-200/40 dark:border-gray-700/40">
                            <span class="text-sm text-gray-500 dark:text-gray-400 font-medium">Total abonado</span>
                            <span
                                class="font-bold text-green-600 dark:text-green-400 text-base leading-tight">${{ number_format($totalPagado, 2) }}</span>
                        </div>
                        <div class="flex justify-between items-center py-0.5">
                            <span
                                class="text-sm text-gray-500 dark:text-gray-400 font-bold uppercase tracking-wider">Saldo
                                Restante</span>
                            <span
                                class="font-black text-lg text-emerald-600 dark:text-emerald-400 leading-tight">${{ number_format($saldoTotalRestante, 2) }}</span>
                        </div>
                    </div>
                </div>
                ...
                @if($esLiquidado)
                    <div
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h5
                                    class="font-bold text-xs text-emerald-800 dark:text-emerald-300 uppercase tracking-wide">
                    ...
                    <div class="grid grid-cols-1 xl:grid-cols-2 gap-6 pt-4 border-t dark:border-gray-700">
                        ...
                        <div class="space-y-2.5">
                            ...
                            <div class="space-y-1.5 max-h-[360px] overflow-y-auto pr-1">
                                @foreach($loan->installments as $installment)
                                    ...
                                    <div
                                        class="flex justify-between items-center py-1.5 px-3 rounded-xl border text-sm transition
                                        ...
                                        <div class="flex items-center gap-2.5">
                                            <span
                                                class="font-bold text-gray-900 dark:text-white text-sm">${{ number_format($installment->amount_due, 2) }}</span>

                                            @if($esPagada)
                                                <span
                                                    class="px-1.5 py-0.5 bg-emerald-100 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-300 rounded text-[10px] font-bold">PAGADA</span>

                                                @if(!$esLiquidado)
                                                    <form
                                                        action="{{ route('installments.destroyPayment', [$loan->id, $installment->id]) }}"
                                                        method="POST"
                                            ...
                                            @else
                                                @if($esVencida)
                                                    <div class="flex items-center gap-1">
                                                        ...
                                                        @endif
                                                        <span
                                                            class="px-1.5 py-0.5 bg-red-100 text-red-700 dark:bg-red-900/60 dark:text-red-300 rounded text-[10px] font-bold">VENCIDA</span>
                                                    </div>
                                                @else
                                                    <span
                                                        class="px-1.5 py-0.5 bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-300 rounded text-[10px] font-bold">PENDIENTE</span>
                                                @endif
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="overflow-x-auto max-h-[360px] overflow-y-auto">
                                <table class="w-full text-left text-xs border-collapse">
                                    <thead>
                                        <tr
                                            ...
                                            <th class="py-1.5 pb-2 font-normal text-xs uppercase tracking-wider">Concepto /
                                                Detalle</th>
                                            <th class="py-1.5 pb-2 font-normal text-right text-xs uppercase tracking-wider">
                                                Abono</th>
                                            <th class="py-1.5 pb-2 font-normal text-right text-xs uppercase tracking-wider">
                                                Saldo</th>
                                        </tr>
                                    ...
                                    <tbody
                                        ...
                                        @foreach($loan->installments as $installment)
                                            ...
                                            <tr>
                                                ...
                                                <td
                                                    class="py-2 text-right font-bold text-emerald-600 dark:text-emerald-400 text-xs">
                                ...
                                @if(!$tieneMovimientos)
                                    <div
                                        class="text-center py-4 text-gray-400 dark:text-gray-500 text-xs italic border-t border-gray-100 dark:border-gray-800/60 mt-1">
                            ...
                            <div class="flex justify-end items-center gap-6 pt-2 mt-1 text-sm font-bold">
                                ...
                                <span
                                    class="text-amber-500 text-sm font-black">${{ number_format($saldoTotalRestante, 2) }}</span>
                            </div>
                        </div>
